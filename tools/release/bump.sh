...
# Mirrors the versionCode + versionName formula in buildSrc/src/main/java/ProjectConfig.kt.
#
# This project is rc-only: the version name is always "<M>.<m>.<p>-rc<build>" and
# version.properties holds only major/minor/patch/build (no type field).
...
Options (for plan/write):
  ...
  --version-override=<M.m.p-rcN>        (overrides bump-kind)
  --expected-current=<M.m.p-rcN>        (fail if current version differs)
...
parse_name() {
  # Sets globals: pn_major, pn_minor, pn_patch, pn_build
  local name="$1" label="$2"
  if [[ ! "$name" =~ ^([0-9]{1,2})\.([0-9]{1,2})\.([0-9]{1,2})-rc([0-9]{1,2})$ ]]; then
    die "$label does not match <M.m.p-rcN>: '$name'"
  fi
  pn_major="${BASH_REMATCH[1]}"
  pn_minor="${BASH_REMATCH[2]}"
  pn_patch="${BASH_REMATCH[3]}"
  pn_build="${BASH_REMATCH[4]}"
...
format_name() {
  local major="$1" minor="$2" patch="$3" build="$4"
  echo "${major}.${minor}.${patch}-rc${build}"
}
...
# Each of the four keys must appear exactly once on its own line.
expect_one() {
...
cur_name=$(format_name "$cur_major" "$cur_minor" "$cur_patch" "$cur_build")
...
new_name=$(format_name "$new_major" "$new_minor" "$new_patch" "$new_build")
